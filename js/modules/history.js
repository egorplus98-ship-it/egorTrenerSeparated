        ...
        grouped[date].forEach(w => {
            ...
            exDiv.innerHTML = `
                <div style="display:flex;justify-content:space-between;">
                    <b>🏋️ ${w.exercise}</b>
                    <button class="round-delete delete-workout-btn" data-id="${w.id}">🗑</button>
                </div>${setsHtml}`;
...
export function setupCaloriesEdit() {
    document.getElementById('saveCaloriesBtn')?.addEventListener('click', async () => {
        if (state.currentEditWorkoutId) {
            ...
            state.trainingHistory = state.trainingHistory.map(w => { 
...
export function renderFoodHistory() {
    ...
    let grouped = {};
    state.foodEntries.slice().sort((a, b) => new Date(b.date) - new Date(a.date)).forEach(f => { 
    ...
    for (let date in grouped) {
        ...
        dayDiv.innerHTML = `
            <div class="history-food-header" data-date="${date}">
                <span>📅 ${formatDateToDMY(date)}</span>
                <div class="action-buttons">
        ...
        grouped[date].forEach(f => {
            ...
            fDiv.innerHTML = `
                <div style="display:flex;justify-content:space-between;">
                    <b>🍽 ${f.name}</b>
                    <button class="round-delete delete-food" data-id="${f.id}">🗑</button>
                </div>
    ...
    document.querySelectorAll('.history-food-header').forEach(header => { 
        header.addEventListener('click', (e) => { 
    ...
    document.querySelectorAll('.delete-food-day').forEach(btn => { 
    ...
    document.querySelectorAll('.delete-food').forEach(btn => btn.addEventListener('click', async (e) => { 
        e.stopPropagation(); 
