...
export function updateProgressChart() {
    ...
    state.customExercises.forEach(function(ex) { 
        var o = document.createElement('option'); o.value = ex; o.textContent = ex; select.appendChild(o); 
    });
    ...
    var filtered = state.trainingHistory.filter(function(e) { return e.exercise === exercise; }).sort(function(a,b) { return new Date(a.date) - new Date(b.date); });
...
export function updateMaxChart() {
    ...
    var prev = select.value;
    select.innerHTML = '';
    state.customExercises.forEach(function(ex) { var o = document.createElement('option'); o.value = ex; o.textContent = ex; select.appendChild(o); });
    if (prev && state.customExercises.indexOf(prev) !== -1) select.value = prev;
    else if (state.customExercises.length > 0) select.value = state.customExercises[0];
    var exercise = select.value;
    if (!exercise || !state.trainingHistory.length) return;
    ...
    filtered.forEach(function(e) {
        ...
        e.sets.forEach(function(s) { if (s.weightType !== 'bw' && s.weight > ms.weight) ms = { weight: s.weight, reps: s.reps, effort: s.effort||0 }; });
        if (ms.weight > 0 && (!dateMap.has(e.date) || dateMap.get(e.date).weight < ms.weight)) dateMap.set(e.date, ms);
    });
    ...
    if (labels.length && data.some(function(d) { return d > 0; })) {
        charts.max = new Chart(ctx, { type: 'line', data: { labels: labels, datasets: [{ label: 'Макс. вес ' + exercise + ' (кг)', data: data, borderColor: '#ff7b2c', backgroundColor: 'rgba(255,123,44,0.15)', fill: false, tension: 0.25 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: '#eef2ff' } }, tooltip: { callbacks: { label: function(ctx) { var d = tt[ctx.dataIndex]; return d ? d.weight + ' кг × ' + d.reps + ' (💪' + d.effort + ')' : ctx.raw + ' кг'; } } } }, scales: { x: { ticks: { color: '#c4c9e0' }, grid: { color: 'rgba(255,255,255,0.06)' } }, y: { ticks: { color: '#c4c9e0' }, grid: { color: 'rgba(255,255,255,0.06)' } } } } });
    }
}
...
export function updateTonnageChart() {
    ...
    var dateMap = new Map();
    filtered.forEach(function(e) {
        var ton = e.sets.reduce(function(t, s) { return s.weightType === 'bw' ? t : t + (s.weight * s.reps); }, 0);
    ...
    if (labels.length && data.some(function(d) { return d > 0; })) {
        charts.tonnage = new Chart(ctx, { type: 'bar', data: { labels: labels, datasets: [{ label: 'Тоннаж ' + exercise + ' (кг)', data: data, backgroundColor: 'rgba(46,204,113,0.6)', borderColor: '#2ecc71', borderWidth: 1 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: '#eef2ff' } } }, scales: { x: { ticks: { color: '#c4c9e0' }, grid: { color: 'rgba(255,255,255,0.06)' } }, y: { ticks: { color: '#c4c9e0' }, grid: { color: 'rgba(255,255,255,0.06)' } } } } });
    }
}
...
export function updateFoodCharts() {
    ...
    state.foodEntries.forEach(function(f) { 
        ...
        grouped[f.date].kcal += f.kcal; grouped[f.date].protein += f.protein; grouped[f.date].fat += f.fat; grouped[f.date].carbs += f.carbs; 
    });
    ...
    if (labels.length) {
        var k = document.getElementById('kcalChart'), p = document.getElementById('proteinChart'), f = document.getElementById('fatChart'), c = document.getElementById('carbsChart'), b = document.getElementById('bjuChart');
        if (k) charts.kcal = new Chart(k, { type: 'line', data: { labels: labels, datasets: [{ label: 'Калории', data: dates.map(function(d) { return grouped[d].kcal; }), borderColor: '#ff7b2c', fill: false }] }, options: { responsive: true, maintainAspectRatio: false } });
...
export function updateWeightChart() {
    ...
    if (sorted.length) weightChart = new Chart(ctx, { type: 'line', data: { labels: sorted.map(function(w) { return formatDateToDMY(w.date); }), datasets: [{ label: 'Вес (кг)', data: sorted.map(function(w) { return w.weight; }), borderColor: '#4c9aff', fill: false }] }, options: { responsive: true, maintainAspectRatio: false } });
}
